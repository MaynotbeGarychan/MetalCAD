           ...
           switch s(1).type
               ...
               case '()'
                   ...
                   elseif length(s) == 3 && strcmp(s(2).type,'.') && strcmp(s(3).type,'()')
                        ...
                        prop = s(2).subs; indices1 = s(1).subs{:}; indices2 = s(3).subs{:};
                        if ~ismember(prop, properties(obj)); error('Property %s does not exist in the %s class.', prop, class(obj)); end
                        if iscell(obj.(prop)(indices1))
                            acell = obj.(prop)(indices1);
                            acell{1}(indices2) = varargin{:};
                            obj.(prop)(indices1) = acell;
                        else
                            obj(indices1).(prop)(indices2) = varargin{:};
                        end
                   else
